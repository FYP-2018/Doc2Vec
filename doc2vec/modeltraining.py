...
with smart_open('sougo_sohunews_sw.txt', 'rb', encoding='utf-8') as alldata:
...
# you need to adjust the hyper-parameters, e.g. vector_size, epochs, alpha
cores = multiprocessing.cpu_count()
simple_models = []

for size in Parameter.vector_size:
    for iteration in Parameter.epoch:
        simple_models.append(Doc2Vec(dm=0, vector_size=size, negative=5, hs=0, min_count=2, sample=0,
                                     epochs=iteration, workers=cores))
    # # PV-DBOW plain
...
for model in simple_models:
    ...
    model.save('models/%s.d2v' % Parameter.model_count)
    Parameter.model_count += 1
